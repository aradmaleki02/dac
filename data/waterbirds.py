...
def get_waterbird_dataloader(split, transform, path, batch_size, mask_path=None, get_mask=False, get_names=False):
    ...
    dataset = WaterbirdDataset(split=split, path=path, mask_path=mask_path, transform=transform, get_names=get_names,
                               get_mask=get_mask)
    if not split == 'train':
        print(split)
...
def get_waterbird_loaders(path, batch_size, mask_path=None, get_mask=False, get_names=False):
    ...
    trainloader = get_waterbird_dataloader('train', t_tst, path, batch_size, mask_path=mask_path, get_mask=get_mask,
                                           get_names=get_names)
    valloader = get_waterbird_dataloader('val', t_tst, path, batch_size, mask_path=mask_path, get_mask=get_mask,
                                         get_names=get_names)
    testloader = get_waterbird_dataloader('test', t_tst, path, batch_size, mask_path=mask_path, get_mask=get_mask,
                                          get_names=get_names)
